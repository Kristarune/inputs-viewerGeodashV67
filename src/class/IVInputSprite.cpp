...
InputSprite::InputSprite() {
    // SDK 5.x: listen on IVSettingEvent for KeyAppearance (color handled by BackgroundSprite)
    m_settingListener = IVSettingEvent().listen([this](SettingEventType type) {
...
bool InputSprite::init(PlayerInputNode* inputNode, PlayerButton button, char const* playerText) {
    ...
    // SDK 5.x: listenForSettingChanges requires explicit type
    listenForSettingChanges<std::string>("cps-calculation", [this](std::string value) {
        this->setCPSMode(convertCPSCalculation(value));
    });
...
void InputSprite::updateButtonAppearance() {
    ...
    if (m_inputNode->getLevelSettings().showTotalInputs ^ m_inputNode->getLevelSettings().showCPS) {
        m_textScale = 0.55f;
        static_cast<AnchorLayoutOptions*>(m_totalInputsText->getLayoutOptions())->setOffset(ccp(0.f, 8.f));
        static_cast<AnchorLayoutOptions*>(m_cpsText->getLayoutOptions())->setOffset(ccp(0.f, 8.f));
    } else if (m_inputNode->getLevelSettings().showTotalInputs && m_inputNode->getLevelSettings().showCPS) {
        m_textScale = 0.45f;
        static_cast<AnchorLayoutOptions*>(m_totalInputsText->getLayoutOptions())->setOffset(ccp(0.f, 11.5f));
        static_cast<AnchorLayoutOptions*>(m_cpsText->getLayoutOptions())->setOffset(ccp(0.f, 5.f));
    }
